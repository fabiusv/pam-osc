...
function CreateCheckBoxDialog(displayHandle)
    ...
    if type(GetDisplayByIndex) == "function" then
        display = GetDisplayByIndex(displayIndex)
    end
    if display == nil then
        display = Root().GraphicsRoot.PultCollect:Ptr(1).DisplayCollect:Ptr(displayIndex)
    end
    ...
    -- Keep the X-Touch timecode layout configurable. The actual PageID
    -- initialization remains intentionally disabled in pam-OSC-Module.js.
    local expandTimecodeIcon = settingsGrid:Append("Button")
    expandTimecodeIcon.Text = ""
    expandTimecodeIcon.Anchors = {
        left = 0,
        right = 0,
        top = 5,
        bottom = 5
    }
    expandTimecodeIcon.Icon = "Time"
    expandTimecodeIcon.Margin = {
        left = 0,
        right = 2,
        top = 2,
        bottom = 2
    }
    expandTimecodeIcon.HasHover = "No"

    local checkBox5 = settingsGrid:Append("CheckBox")
    checkBox5.Anchors = {
        left = 1,
        right = 9,
        top = 5,
